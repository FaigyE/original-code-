...
import type { CustomerInfo, Note } from "./types"
...
export function ReportProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (storedToiletCount) setToiletCount(JSON.parse(storedToiletCount))
    if (storedNotes) setNotes(JSON.parse(storedNotes))
    if (storedLetterText) setLetterText(JSON.parse(storedLetterText))
    if (storedReportTitle) setReportTitle(JSON.parse(storedReportTitle))
    if (storedSignatureName) setSignatureName(JSON.parse(storedSignatureName))
    if (storedSignatureTitle) setSignatureTitle(JSON.parse(storedSignatureTitle))
    if (storedRePrefix) setRePrefix(JSON.parse(storedRePrefix))
    if (storedDearPrefix) setDearPrefix(JSON.parse(storedDearPrefix))
    if (storedSectionTitles) setSectionTitles(JSON.parse(storedSectionTitles))
    if (storedCoverImage) setCoverImage(JSON.parse(storedCoverImage))
    if (storedCoverImageSize) setCoverImageSize(JSON.parse(storedCoverImageSize))
  }, [])
